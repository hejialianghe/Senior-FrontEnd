...
因为我们用worker是限制的，不是本地跑一个html就行，`new Worker("worker.js")`里的参数不能通过本地路径去取的，不支持file协议；需要起一个静态资源服务，我们可以用`:tomato:`。

```bash
npm install -g :tomato: # 下载

-:tomato: start --server # 启动，注意启动服务要在index.html这个文件夹
```
xw
