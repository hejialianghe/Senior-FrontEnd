...
从上面可知,我们用 Dep 类去存放依赖,现在我们使数据变的可观测,又知道了`何时去存放依赖`,`何时又去通知依赖更新视图`,`在哪存放依赖`,现在我们不知道的是`依赖是谁?`
...
我们对 data return 出来的这个对象转换成响应式进行观测；我们获取数组时，肯定是 obj.test;这样的话肯定会走 obj 的 getter 中，所以我们收集依赖也是在
getter 中
