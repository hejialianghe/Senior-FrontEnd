...
## 2.5 高阶函数
 ...
 ### 2.8.4 第三方库实现拷贝
  ...
  🔥什么是子序列？
   ...
   一个字符窜的子序列是指一个新的字符窜，在不改变原序列相对位置的情况下删除原序列若干个元素（也可以不删除）之后得到的新序列，这个序列就原序列的子序列
  ...
  🔥参考文档：[点击我](https://leetcode-cn.com/problems/longest-common-subsequence/)

  🔥案例： 求最长公共子序列

  一般在解决算法的时候，一般有四种算法思想，分治法、动态规划、回溯法、贪心算法，这一题适合动态规划来做
  ``` javascript
      const lcsamples = {
        string1: "abcde",
        string2: "ace",
  ...
  🔥动态规划（英语：Dynamic programming，简称 DP））
   ...
   动态规划我们拿笔画一画，一个作为横轴一个作为纵轴，我们以2个字符串为例子，那么abcde作为横轴，ace作为纵轴
   ![](~@/jsasvanced/lcs1.png)
   ![](~@/jsasvanced/lcs2.png)
   ![](~@/jsasvanced/lcs3.png)
   ![](~@/jsasvanced/lcs4.png)
...
### 2.9.6  莱温斯坦距离问题
  ...
  🔥参考文档：[点击我](https://leetcode-cn.com/problems/edit-distance/)
  
  🔥案例：
  假设有2个字符窜，第一行从空到s过程是增，E到空是删，E到s是改，这是编辑的3种情况
  ![](~@/jsasvanced/levenshtein1.png)
  下图黑框代表任意的字符窜，前面不管是什么，我们先比较最后一个，如果最后一个字符窜不相等，在去比较前面的最优子结构加1，相等不加1，按照最优的子结构不断的迭代下去
