...
概念：在链式存储中，每个节点是一个对象，该对象包含三个属性：value、left 和 right。left 和 right 指向子节点，如果某个子节点不存在，则对应的指针为 null。
...
概念：在顺序存储中，二叉树的节点按照从上到下、从左到右的顺序存储在一个数组中。数组的每个元素对应一个二叉树的节点，数组的索引表示节点在树中的位置关系。

节点与数组索引的关系

- 根节点：根节点总是存储在数组的第一个位置，即索引为 0 的位置。
- 左子节点：对于索引为 i 的节点，左子节点的位置是 2 * i + 1。
- 右子节点：对于索引为 i 的节点，右子节点的位置是 2 * i + 2。
- 父节点：对于索引为 i 的节点，其父节点的位置是 (i - 1) // 2（// 表示整除）。
...
总结:
- 链式存储使用对象和指针的方式，节点之间通过指针连接，灵活性高，但占用额外的指针空间。
- 顺序存储使用数组，适合完全二叉树或满二叉树，访问速度快，但对于稀疏树会导致空间浪费。
...
- 前序遍历（Preorder Traversal）：
  - 顺序: 先访问根节点 -> 遍历左子树 -> 遍历右子树。
  - 解释: 在前序遍历中，首先访问当前节点（根节点），然后递归地对左子树进行前序遍历，最后对右子树进行前序遍历。

- 中序遍历（Inorder Traversal）：先遍历左子树，然后访问根节点，最后遍历右子树。
  - 顺序: 先遍历左子树 -> 访问根节点 -> 遍历右子树。
  - 解释: 在中序遍历中，首先递归地遍历左子树，接着访问当前节点（根节点），然后递归地遍历右子树。

- 后序遍历（Postorder Traversal）：先遍历左子树，然后遍历右子树，最后访问根节点。
  - 顺序: 先遍历左子树 -> 遍历右子树 -> 访问根节点。
  - 解释: 在后序遍历中，首先递归地遍历左子树，接着递归地遍历右子树，最后访问当前节点（根节点）

- 层序遍历（Level Order Traversal）：从根节点开始，逐层从左到右访问所有节点。
  - 顺序: 从根节点开始，逐层从左到右依次访问每一层的所有节点。
  - 特点: 层序遍历是按层次逐层访问节点的，因此每一层的节点在输出时都是按从左到右的顺序。
:::warning
